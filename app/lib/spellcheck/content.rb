  ...
  def normalized_content
    @normalized_content ||= begin
      @content
        ...
        .gsub(/\#([a-f0-9]{2,6})[,; !<"')]/i) { |phrase| ' ' * phrase.length } # colour hashes
        .gsub(%r{sha512-([a-z0-9+/]{60,})==}i) { |phrase| ' ' * phrase.length } # Remove any sha512 looking things
        .gsub(%r{https://([a-z0-9+/]{14,20}).cloudfront.net/([a-z0-9+/]{36,52})/([a-z0-9+/]{4,6})/}i) { |phrase| ' ' * phrase.length } # Remove any x-amz-cf-id looking things
        .gsub(/ ([a-z0-9_\-]{50,})==/i) { |phrase| ' ' * phrase.length } # Remove any Netlify CDN references
        ...
        .gsub(/\{([a-z0-9\-]{36,})\}/i) { |phrase| ' ' * phrase.length } # Remove any windows hashes looking things
