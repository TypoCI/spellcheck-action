...
RSpec.describe Github::CheckSuites::UpdateRemoteJob, type: :job do
  let(:github_check_suite) { build(:github_check_suite, :analysed_no_typos) }
  let(:instance_class) { described_class.new }
  let(:github_octokit_service) { double :github_octokit_service }

  before do
    allow(instance_class).to receive(:github_octokit_service).and_return(github_octokit_service)
  ...
  describe '#perform' do
    ...
    context 'With >50 annotations' do
      ...
      before do
        github_check_suite.annotations = annotations
      end
      ...
      it do
        ...
        expect(github_octokit_service).to receive(:update_check_run).twice
  ...
  describe '#output_summary_actions' do
    ...
    context 'custom configuration file is invalid' do
      ...
      it { is_expected.to include('**Action Required:** We were unable to parse your `.typo-ci.yml` file. Please review') }
    end
  end
