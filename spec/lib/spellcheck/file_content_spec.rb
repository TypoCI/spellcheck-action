...
RSpec.describe Spellcheck::FileContent do
  ...
  describe '#annotations' do
    ...
    context 'files without spelling errors' do
      Rails.root.join('spec', 'fixtures', 'files', 'correctly_spelt_files').glob('*').each do |pathname|
  ...
  describe '#invalid_words' do
    ...
    context 'Brazilian Dictionary' do
      ...
      context 'files without spelling errors' do
        Rails.root.join('spec', 'fixtures', 'files', 'correctly_spelt_files_in_pr').glob('*').each do |pathname|
          context pathname.to_s do
            let(:contents) { pathname.read }
            let(:path) { pathname.to_s }
            it { expect(subject.collect(&:word)).to eq([]) }
  ...
  describe '#normalized_content' do
    ...
    context 'contents contains a sha512 hash' do
      let(:contents) { 'integrity sha512-E6uQ4kRrTX9URN9s/lIbqTAztwEP+vzVrcmHE8EQ9YnuT9J8Es5Wrd8n9BKg1a0oZ5EgEke/EQFgUsp18dSTBw==' }
    ...
    context "contents contains shouldn't" do
      let(:contents) { '         * 4. After we’ve iterated through every symbol of every module, any symbol left in Bucket C means that step 3 didn’t' }
      it { is_expected.to eq('              After we’ve iterated through every symbol of every module  any symbol left in Bucket C means that step   didn’t') }
    end
    ...
    context 'contents is not UTF-8 encoding' do
      let(:contents) { 'V by Example é uma introdução direta ao V usando exemplos de programas anotados.'.encode('ISO-8859-1') }
