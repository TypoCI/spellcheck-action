...
RSpec.describe Spellcheck::GitDiff do
  ...
  describe '#message(invalid_word)' do
    subject { described_class.new(patch, path: path, configuration: Spellcheck::Configuration.new).send(:message, invalid_word) }
    ...
    context 'Invalid word is: recieve' do
      ...
      it { is_expected.to eq("\"recieve\" is a typo. Did you mean \"receive\"? I remember this with the rhyme \"I before E, except after C unless it's one of 923 words spelled cie\".") }
    end

    context 'Invalid word is: Samesies' do
      let(:invalid_word) { Spellcheck::Word.new('Samesies') }

      it { is_expected.to eq("I don't know this word. \"Samesies\"? From context, I believe it means \"Proudly Uneducated\".") }
    end
  end
end
